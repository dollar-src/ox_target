...
local function startTargeting()
    ...
    CreateThread(function()
        ...
        while state.isActive() do
            ...
            utils.drawDui()  -- Draw DUI card + connecting line each frame
    ...
    while state.isActive() do
        ...
        if hasTarget and (zonesChanged or entityChanged and hasTarget > 1) then
            ...
            utils.hideDui()  -- Hide DUI card when leaving target
        ...
        if newOptions then
            ...
            elseif menuChanged or hasTarget ~= 1 and hidden ~= totalOptions then
                ...
                local duiOptions = {}
                for t, typeOptions in pairs(options) do
                    for i = 1, #typeOptions do
                        local o = typeOptions[i]
                        if not o.hide then
                            duiOptions[#duiOptions + 1] = { 
                                icon = o.icon, 
                                label = o.label, 
                                type = t, 
                                id = i, 
                ...
                utils.showDui(targetCoords, duiOptions)
            end
    ...
    utils.hideDui()  -- Cleanup DUI on target end
...
RegisterNUICallback('updateSettings', function(data, cb)
    cb(1)
    if data then
        utils.updateSettings(data)
    end
end)

RegisterCommand(Config and Config.SettingsCommand or 'targetsettings', function()
    if IsPauseMenuActive() then return end
    
    state.setNuiFocus(true, true, false)
    SendNuiMessage('{"event": "visible", "state": true}')
    SendNuiMessage('{"event": "openSettings"}')
end, false)
